...
class DataGrid
{
    ...
    function drawControlPanel(){       
        ...
        if($this->filtering_allowed || $this->exporting_allowed || $this->printing_allowed){
            ...
            if($this->filtering_allowed && ($this->mode == "view") && ($req_mode != "update") && ($req_mode != "delete")){
                echo "<td align='right' width='20px'><a style='cursor:pointer;' onClick='window.location.href=self.location;' class='".$this->css_class."_class_a'><img src='".$this->directory."images/".$this->css_class."/recycle.gif' alt='".$this->lang['refresh_page']."' title='".$this->lang['refresh_page']."' /></a>";
            }        
            echo "</tr>";
            echo "</table>";
        }
    ...
    function drawTabular(){
        ...
        for($r = $this->row_lower; (($r >=0 && $this->row_upper >=0) && ($r < $this->row_upper) && ($r < ($this->row_lower + $this->req_page_size))); $r++){            
            ...
            if(($r == $this->row_lower) && ($this->rid == -1) && ($req_mode == "add")){
                ...
                $this->drawModeButton("cancel", $cancel_url, $this->lang['cancel'], $this->lang['cancel'], "cancel.gif", "\"return ".$this->unique_prefix."verifyCancel('".$cancel_url."'); javascript:window.location.href='".$this->HTTP_URL.$cancel_url."'\"", false, $this->nbsp, "");
                $this->colClose();                
    ...
    function pagingSecondPart($lu_paging=false, $upper_br, $lower_br, $type="1"){
        ...
        if($this->pages_total >= 1){
            ...
            $text .= "\n<script type='text/javascript'>\n<!--\n";
            $text .= "function ".$this->unique_prefix."setPageSize".$type."(){window.location.href = '$href_string&".$this->unique_prefix."page_size='+document.frmPaging$this->unique_prefix".$type.".page_size".$type.".value;}";
            $text .= "\n//-->\n</script>\n";
    ...
    function setCommonJavaScript(){
        ...
        echo "\n<script type='text/javascript'>\n<!--";                
        if(isset($this->modes['cancel'][$this->mode]) && $this->modes['cancel'][$this->mode]){
            echo "\nfunction ".$this->unique_prefix."verifyCancel(param){if(confirm(\"".$this->lang['cancel_creating_new_record']."\")){window.location.href=param;} else { return false;}};";
        }        
        echo "//-->\n</script>\n";               
    }
    ...
    function setCommonJavaScriptEnd(){
        ...
        // set verify JS functions
        if(isset($this->modes['delete'][$this->mode]) && $this->modes['delete'][$this->mode]){
            echo "\nfunction ".$this->unique_prefix."verifyDelete(param){if(confirm(\"".$this->lang['delete_this_record']."\")){window.location.href=param;} else { window.event.cancelBubble = true; return false;}};";
        }        
        echo "\n//-->\n";
        echo "</script>";        
    }
